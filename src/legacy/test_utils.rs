...
use crate::legacy::CrdtRange;
use crate::test_utils::SimpleSpan;
pub use crate::test_utils::{Action, AnnotationType};
...
impl Actor {
    ...
    #[inline(always)]
    pub fn un_annotate(&mut self, range: impl RangeBounds<usize>, type_: &str) {
    ...
    pub(crate) fn get_annotations(&mut self, range: impl RangeBounds<usize>) -> Vec<SimpleSpan> {
        let mut spans = vec![];
        for span in self
            .range
    ...
    #[allow(unused)]
    pub fn check_eq(&mut self, other: &mut Self) {
...
fn make_spans(spans: &[(Vec<&str>, usize)]) -> Vec<SimpleSpan> {
    spans
        .iter()
